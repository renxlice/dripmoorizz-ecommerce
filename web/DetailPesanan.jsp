...
<%
    ...
    String confirmStatus = request.getParameter("confirm");
    ...
    String statusPesanan = "Belum Dikonfirmasi";
    ...
    try {
        ...
        ps = conn.prepareStatement("SELECT status FROM data_pesanan WHERE id_pesanan = ?");
        ps.setString(1, idPesanan);
        rs = ps.executeQuery();
        if (rs.next()) {
            statusPesanan = rs.getString("status");
        }
        rs.close();
        ps.close();
...
<div class="content">
    <div class="card-glass">
        <h3 class="mb-2">📦 Detail Pesanan ID: <%= idPesanan %></h3>
        <p class="mb-4">📝 Status: <strong><%= statusPesanan %></strong></p>
        ...
        <% if (!"Dikonfirmasi".equalsIgnoreCase(statusPesanan)) { %>
            <form action="KonfirmasiPesanan.jsp" method="post" class="mt-3 text-end">
                <input type="hidden" name="id_pesanan" value="<%= idPesanan %>"/>
                <button type="submit" class="btn btn-success">✅ Konfirmasi Pesanan</button>
            </form>
        <% } else { %>
            <p class="text-success fw-bold text-end">✅ Pesanan sudah dikonfirmasi.</p>
        <% } %>
...
<% if ("success".equals(confirmStatus)) { %>
<script>
    Swal.fire({
        icon: 'success',
        title: 'Pesanan Dikonfirmasi',
        text: 'Pesanan berhasil dikonfirmasi dan akan segera diproses!',
